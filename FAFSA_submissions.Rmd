...
fafsa <- read_xls("data/FAFSA/2024 reports/FL-FAFSA submissions-2024-03-08.xls",
  ...
  bind_rows(read_xls("data/FAFSA/2024 reports/FL-FAFSA submissions-2024-03-22.xls",
                  sheet = "FL School Level Data",
                  skip = 4, 
                  col_names = F, 
                  na = "<5") %>% 
              mutate(date = ymd("2024-03-22")))
...
subs_dis_pov <- fafsa_subs %>%
  filter(!is.na(submission_rate)) %>%
  left_join(lunch_status) %>% 
  filter(date == max(fafsa_subs$date))

frl_rate_quantiles <- quantile(subs_dis_pov$rate_wo_multiplier[which(subs_dis_pov$class == "2024")], seq(.1,1,.1), na.rm = T)

lunch_status_qtl <- subs_dis_pov %>% 
  group_by(district) %>% 
  mutate(dist_ave_rate = mean(rate_wo_multiplier),
         rate_g_ave = rate_wo_multiplier > dist_ave_rate,
         rate_g_X = rate_wo_multiplier >= .5,
         pov_quantile = case_when(
           rate_wo_multiplier <= frl_rate_quantiles[1] ~ "10th", 
           rate_wo_multiplier <= frl_rate_quantiles[2] ~ "9th",
           rate_wo_multiplier <= frl_rate_quantiles[3] ~ "8th", 
           rate_wo_multiplier <= frl_rate_quantiles[4] ~ "7th", 
           rate_wo_multiplier <= frl_rate_quantiles[5] ~ "6th", 
           rate_wo_multiplier <= frl_rate_quantiles[6] ~ "5th", 
           rate_wo_multiplier <= frl_rate_quantiles[7] ~ "4th", 
           rate_wo_multiplier <= frl_rate_quantiles[8] ~ "3rd", 
           rate_wo_multiplier <= frl_rate_quantiles[9] ~ "2nd", 
           rate_wo_multiplier <= frl_rate_quantiles[10] ~ "1st", 
           T ~ NA)) %>% 
  mutate(district_number = as.numeric(dist_number),
         school_number = as.numeric(schl_number)) %>% 
    select(district_number, school_number, school_name, 
           ...
           lunch_quantile = pov_quantile, 
           students_lunch = number_of_students_denominator, 
           submission_rate,
           class,
           district, 
           students_gr12 = students)

lunch_status_qtl %>% 
  group_by(lunch_quantile, class) %>% 
  filter(!is.na(lunch_quantile)) %>% 
  summarise(count = n(), 
            min = min(lunch_rate_wo_multiplier, na.rm = T),
            max = max(lunch_rate_wo_multiplier, na.rm = T),
            mean = mean(lunch_rate_wo_multiplier, na.rm = T))
...
lunch_status_qtl %>% 
  ungroup() %>% 
  group_by(lunch_quantile, class) %>% 
  summarise(submission_rate = weighted.mean(submission_rate, students_gr12),
            count = n()) %>% 
  arrange(class) %>% 
  filter(!is.na(lunch_quantile)) %>% 
  arrange(class) %>% 
  write_csv("data/FAFSA/output/lunch_status_quantile.csv", na = "")
...
subs_sch_race <- lunch_status_qtl %>% 
  left_join(school_race) %>% 
  mutate(share_hisp = hispanic_latino/students_gr12, 
         maj_hisp = class == "2024" & share_hisp > .67) %>% 
  group_by(school_number, district_number) %>%
  mutate(maj_hisp_keep = max(maj_hisp)) %>% 
  group_by(maj_hisp_keep, class) %>% 
  summarise(submission_rate = weighted.mean(submission_rate, students_gr12), 
            county = n())
